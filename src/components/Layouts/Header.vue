        ...
        <div class="flex justify-between items-center bg-[#97b8e0] my-3 p-5 rounded-full">
          ...
          <div class="hidden lg:block">
            <ul class="flex flex-row gap-8">
              <li v-for="(navbarlink, index) in navbarlinks" :key="index">
                <RouterLink :to="{ name: navbarlink.link }">
                  {{ navbarlink.name }}
                </RouterLink>
              </li>
            </ul>
          ...
          <div class="hidden lg:block">
            <div class="flex items-center gap-3">
              <span class="bg-white text-blue-600 p-3 rounded-full hover:bg-blue-600 hover:text-white"><i class="fa-solid fa-phone-volume"></i></span>
              <span>
                Need Help <br />
          ...
          <div @click="toggleMenu" class="block lg:hidden cursor-pointer bg-white  border border-slate-600 p-3 rounded-full">
            ...
            <ul class="flex flex-col gap-6 text-slate-500 p-6">
              <li v-for="(navbarlink, index) in navbarlinks" :key="index">
                <RouterLink
                  ...
                  @click="toggleMenu"
                  class="block text-lg font-medium hover:text-[#422FBD] transition"
                >
                  > {{ navbarlink.name }}
                </RouterLink>
              </li>
